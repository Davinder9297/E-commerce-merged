...
/** POST: http://localhost:8080/api/removestoreproducts 
 * body: {
        "email": "sahilkumar142002@gmail.com",
        "productIDs": ["65d98e068f61d2603fe62548", "65d98e068f61d2603fe62549", "65d98e068f61d2603fe6254c"]
    }
*/
export async function removestoreproducts(req, res) {
    let sellerID = req.sellerID;
    ...
    try {
        ...
        for (const productID of productIDs) {
            ...
            const index = product.stores.findIndex(store => store.store.toString() === shop._id.toString());
            if (index !== -1) {
                product.stores.splice(index, 1);
                removedProducts.push(productID);
                await product.save().then(() => {
                    try {
                        const shopIndex = shop.products.findIndex(shopProduct => shopProduct.toString() === productID);
                        if (shopIndex !== -1) {
                            shop.products.splice(shopIndex, 1);
                        }
                    } catch (err) {
...
/** POST: http://localhost:8080/api/toogleproductvisibility 
 * body: {
        "email": "sahilkumar142002@gmail.com",
        "productIDs": ["65d98e068f61d2603fe62548", "65d98e068f61d2603fe62549", "65d98e068f61d2603fe6254c"]
    }
*/
export async function toogleproductvisibility(req, res) {
    let sellerID = req.sellerID;
    let { productIDs } = req.body;
    let updatedProducts = [];
    try {
        ...
        for (const productID of productIDs) {
            ...
            const index = product.stores.findIndex(store => store.store.toString() === shop._id.toString());
            if (index !== -1) {
                if (product.stores[index].isVisible == true) {
					product.stores[index].isVisible = false; // Toggle visibility
				} else {
					product.stores[index].isVisible = true; // Toggle visibility
				}
        ...
        res.status(200).json({
            success: true,
            msg: 'Products visibility toggled successfully!',
            updatedProducts,
        });
    } catch (error) {
        console.error(error);
...
export async function productsbystore(req, res) {
	...
	try {
		...
		for (const key in rest.products) {
			let product = rest.products[key];
			
			// Find the index of the store
			const index = product.stores.findIndex(store => store.store.toString() === shop);
			
			// Check if the index is valid
			if (index !== -1) {
				// Update isVisible property based on the store's isVisible value
				product.isVisible = product.stores[index].isVisible;
			}
		}
...
/** PUT: http://localhost:8080/api/updateseller/:sellerID */
export async function updateSeller(req, res) {
	try {
		const {sellerID} = req.params;
		const sellerData = req.body;
        
        // Find the existing seller by ID and update it
		let seller = await sellerModel.findByIdAndUpdate(sellerID, sellerData, { new: true });
		...
		res.status(200).json({ success: true, msg: 'seller updated successfully', data: seller });
	} catch (error) {
		console.error(error);
		res.status(500).json({ success: false, msg: 'Internal server error' });
	}
...
/** delete: http://localhost:8080/api/deleteseller/:sellerId */
export async function deleteseller(req, res) {
	try {
		...
		res.status(200).json({ success: true, msg: 'seller deleted successfully', data: deletedseller });
	} catch (error) {
		console.error(error);
		res.status(500).json({ success: false, msg: 'Internal server error' });
	}
