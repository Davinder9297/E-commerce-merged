...
export async function getdeliveryboys(req, res) {
	try {
        const deliveryboys = await deliveryboyModel.find({}).select('-password');
...
export async function acceptdeliveryboyorder(req, res) {
	try {
		...
		await OrdersModel.updateOne({ _id },
			{ status: "processed"})
        await deliveryboy.save();

        res.status(201).json({success: true, msg: 'Order accepted successfully' });
    } catch (error) {
        console.error(error);
        res.status(500).json({success: false, msg: 'Internal server error' });
    }
...
export async function deliverdeliveryboyorder(req, res) {
	try {
		...
		OrdersModel.updateOne({ _id : orderID },
			{ status: "delivered", delivered_on: new Date()})
			.exec()
			.then(()=>{
...
/** POST: http://localhost:8080/api/pickupdeliveryboyorder 
 * body {
		"orderID": "65e050f4392acdc91e9db224"
	}
*/
export async function pickupdeliveryboyorder(req, res) {
	try {
		const { orderID } = req.body
		...
		OrdersModel.updateOne({ _id : orderID },
			...
			.exec()
			.then(()=>{
				return res.status(201).send({ msg : "Successfully pickuped!"})
			})
			.catch((error)=>{
				throw error;
			})
...
/** PUT: http://localhost:8080/api/updatedeliveryboy/:deliveryboyID */
export async function updatedeliveryboy(req, res) {
	try {
		const {deliveryboyID} = req.params;
		const deliveryboyData = req.body;
        
        // Find the existing deliveryboy by ID and update it
		let deliveryboy = await deliveryboyModel.findByIdAndUpdate(deliveryboyID, deliveryboyData, { new: true });

        // Check if the deliveryboy exists
		if (!deliveryboy) {
			return res.status(404).json({ success: false, msg: 'deliveryboy not found' });
		}

		res.status(200).json({ success: true, msg: 'deliveryboy updated successfully', data: deliveryboy });
	} catch (error) {
		console.error(error);
		res.status(500).json({ success: false, msg: 'Internal server error' });
	}
...
/** delete: http://localhost:8080/api/deletedeliveryboy/:deliveryboyId */
export async function deletedeliveryboy(req, res) {
	try {
		const {deliveryboyId} = req.params;

		// Find the deliveryboy by ID and delete it
		const deleteddeliveryboy = await deliveryboyModel.findByIdAndDelete(deliveryboyId);
		...
		res.status(200).json({ success: true, msg: 'deliveryboy deleted successfully', data: deleteddeliveryboy });
	} catch (error) {
		console.error(error);
		res.status(500).json({ success: false, msg: 'Internal server error' });
	}
