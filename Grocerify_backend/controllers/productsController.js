...
/** PUT: http://localhost:8080/api/updateproduct/:productId */
export async function updateProduct(req, res) {
	try {
		...
		let product = await productModel.findByIdAndUpdate(productId, productData, { new: true });
...
*/
export async function reqaddproduct(req, res) {
	try {
		const productData = req.body
		// If the user has no wishlist, create a new one
        productData.slug = slugify(productData.products_title)
		let product = new productModel({...productData, isVerified: false})
		await product.save()
		res.status(201).json({ success: true, msg: 'Product added successfully' })
	} catch (error) {
		console.error(error)
...
export async function verifiyreqaddproduct(req, res) {
    try {
        const { productID } = req.body
        if (!productID) {
            return res.status(500).json({ success: false, msg: 'Product ID is required' })
        }
        let product = await productModel.findOne({_id: productID})
        productModel.updateOne({ _id: productID }, { isVerified: !product.isVerified })
        return res.status(201).json({ success: true, msg: 'Product Updated Successfully!' })
    } catch (error) {
        console.error(error)
        res.status(500).json({ success: false, msg: 'Internal server error' })
    }
...
export async function getcart(req, res) {
	...
	try {
        ...
        const cart = await cartModel.findOne({_id:userID}).populate('products.product').populate('products.shopID');
