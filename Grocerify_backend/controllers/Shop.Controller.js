...
export async function registerShop(req, res) {
	let {sellerID} = req.seller
	try {
		const shopData = req.body
		shopData.Owner = sellerID
...
export async function approveshop(req, res) {
	...
	try {
		...
		if (approved && !shop.approved) {
			...
			await SellerModel.updateOne({ _id: shop.Owner }, { Shop: shopID })
			registerMail(
				{
					body: {
...
export async function getShopById(req, res) {
	...
	try {
		...
		const orders = await OrdersModel.find({shop: shopID}).populate({
...
export async function ordersbyshop(req, res) {
	...
	try {
		...
		const orders = await OrdersModel.find({shop: sellerData.Shop}).populate({
            path: 'products.productid',
            select: '-stores'
        })
        .populate({
...
/** POST: http://localhost:8080/api/acceptshoporder
 * body {
		"orderID": "65e050f4392acdc91e9db224"
	}
*/
export async function acceptshoporder(req, res) {
	try {
		...
		OrdersModel.updateOne({ _id : orderID },
			...
			.exec()
			.then(()=>{
				return res.status(201).send({ msg : "Successfully Accepeted!"})
			})
			.catch((error)=>{
				throw error;
			})
...
/** POST: http://localhost:8080/api/cancelorder
 * body {
		"orderID": "65e050f4392acdc91e9db224"
	}
*/
export async function cancelOrder(req, res) {
	try {
		...
		OrdersModel.updateOne({ _id : orderID },
			...
			.exec()
			.then(()=>{
				return res.status(201).send({ msg : "Successfully cancelled!"})
			})
			.catch((error)=>{
				throw error;
			})
